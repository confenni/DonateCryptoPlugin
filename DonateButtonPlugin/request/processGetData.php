        ...
        if ($author_agreement == 1 && $reviewer_agreement == 1 && $publisher_agreement == 1) {
          ...
          if ($execGetSubmission) {
            ...
            if ($smart_contract) {
              ...
              if ($exec) {
                $updateSmartContract = "UPDATE smart_contract SET smart_contract_address = :smart_contract_address, percentages_authors = :percentage_authors, percentages_reviewers = :percentage_reviewers, percentages_publisher = :percentage_publisher, expired = :expired WHERE id_submission = :id_submission";

                $stmt = $pdo->prepare($updateSmartContract);
                $stmt->bindParam(':id_submission', $id_submission);
                $stmt->bindParam(':smart_contract_address', $smart_contract);
                $stmt->bindParam(':percentage_authors', $percentage_authors);
                $stmt->bindParam(':percentage_reviewers', $percentage_reviewers);
                $stmt->bindParam(':percentage_publisher', $percentage_publisher);
                $stmt->bindParam(':expired', $expired_timestamp);
                $exec = $stmt->execute();

                if ($exec) {
                  $response['status'] = true;
                  $response['message'] = 'update smart contract successful';
                  $response['data'] = $result;
                } else {
                  $response['status'] = false;
                  $response['message'] = 'update smart contract failed';
                }
              } else {
