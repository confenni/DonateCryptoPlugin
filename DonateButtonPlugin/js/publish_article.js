document.addEventListener('DOMContentLoaded', function () {
    $(document).ready(function () {
        ...
        async function saveIssueFunction() {
            try {
                await fetch(server + path + '/plugins/generic/DonateButtonPlugin/request/publications.php?type=saveIssueId', {
                    method: "POST",
                    body: JSON.stringify({
        ...
        */
        async function getPublications() {
            try {
                await fetch(server + path + '/plugins/generic/DonateButtonPlugin/request/publications.php?type=getDataBySubmissionId&submissionId=' + submission_id)
                    .then(response => response.json())
                    .then(data => {
        ...
        watchCurrentFragment(async value => {
            if (value === "#publication") {
                ...
                let assignIssueButton = $("button.pkpButton.pkpFormField--selectIssue__button");
                // assignIssueButton.prop('disabled', true)
                assignIssueButton.on("click", function () {
                    if (publications.author_agreement == 1 && publications.publisher_agreement == 1 && publications.reviewer_agreement == 1) {
                        checkPublishButton();
                    }
                    checkSaveIssue();
                ...
                function checkSaveIssue() {
                    let modal = $("div.pkp_modal_panel");

                    if (modal.length) {
                        let saveIssue = modal.find("button.pkpButton:contains('Save')");

                        if (saveIssue.length) {
                            let issueValue = $("#assignToIssue-issueId-control");

                            issueValue.on('change', function () {
                                if ($(this).val() !== "") {
                                    issueId = $(this).val();
                                } else {
                                    issueId = 0;
                            ...
                            saveIssue.on('click', async function () {
                                await saveIssueFunction();
                            })
                        } else {
                            setTimeout(checkSaveIssue, 100)
                        }
                    } else {
                        setTimeout(checkSaveIssue, 100)
                    }
